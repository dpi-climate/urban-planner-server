...
def build_csv(csv_file, raw_gdf, prefix, geo_feature_id, csv_feature_id, threshold=None):
    # Load CSV data and prepare DataFrame
    df = pd.read_csv(
        csv_file,
        delimiter=',',
        encoding='utf-8',
    ...
    # Rename feature ID for merging
    df.rename(columns={csv_feature_id: geo_feature_id}, inplace=True)

    # Ensure the geo feature columns are strings for proper merge
    raw_gdf[geo_feature_id] = raw_gdf[geo_feature_id].astype(str)
    df[geo_feature_id] = df[geo_feature_id].astype(str)

    # Merge raw_gdf with the CSV dataframe
    merged_df = raw_gdf.merge(df, on=geo_feature_id, how="left")

    # Select only necessary columns including all time stamp columns
    time_stamp_cols = ["RPL_THEME1", "RPL_THEME2", "RPL_THEME3", "RPL_THEME4", "RPL_THEMES"]
    selected_cols = [geo_feature_id, "geometry"] + time_stamp_cols
    filtered_df = merged_df[selected_cols].copy()

    # Rename the geographic identifier column to UNITID
    filtered_df.rename(columns={geo_feature_id: "UNITID"}, inplace=True)

    # Replace negative values with None for each time stamp column
    for col in time_stamp_cols:
        filtered_df[col] = filtered_df[col].apply(lambda x: None if pd.notnull(x) and x < 0 else x)

    # Export the DataFrame to a single CSV file
    output_file = f"{processed_socio_dir}/{prefix}_combined.csv"
